...
with gr.Blocks() as demo:
    ...
    with gr.Accordion("生成参数", open=False):
        ...
        slider_context_times = gr.Slider(minimum=0, maximum=5, label="上文轮次", value=4,step=1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is_4bit", default=True,action='store_true', help='use 4bit model')
    ...
    #     model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path,device_map=torch.cuda.current_device(),torch_dtype=torch.float16,load_in_8bit=True,trust_remote_code=True)
    #     model.eval()
    # else:
    #     from auto_gptq import AutoGPTQForCausalLM
    #     model = AutoGPTQForCausalLM.from_quantized(args.model_name_or_path,low_cpu_mem_usage=True, device=torch.cuda.current_device(), use_triton=False,inject_fused_attention=False,inject_fused_mlp=False)
